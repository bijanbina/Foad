...
int QRSDetection::Detect(int datum)
{
    detcted_sig.push_back(0); // add a new data to detected signal
    int fdatum, QrsDelay = 0 ;
    int i, newPeak, aPeak ;

    fdatum = QRSFilter(datum,0) ;	/* Filter data. */
    filter_sig.push_back(fdatum);
    ...
    if(aPeak && !preBlankCnt)			// If there has been no peak for 200 ms
    {
        ...
        //detcted_sig[detcted_sig.size()-1] = 512; // MY Code SHow End Of A Peak
    }
    ...
    else	/* Else test for a qrs. */
    {
        ...
        if(newPeak > 0)
        {
            ...
            if(!BLSCheck(DDBuffer, DDPtr, &maxder))
            {
                ...
                {
                    detcted_sig[detcted_sig.size()-1] = 512;
...
int QRSDetection::Peak( int datum, int init )
	{
...
int QRSDetection::mean(int *array, int datnum)
	{
	long sum ;
	int i ;
...
int QRSDetection::thresh(int qmean, int nmean)
	{
	int thrsh, dmed ;
	double temp ;
	dmed = qmean - nmean ;
...
int QRSDetection::BLSCheck(int *dBuf,int dbPtr,int *maxder)
	{
...
int QRSDetection::getCounter()
{
    return detcted_sig.size();
}


vector <double> QRSDetection::getDetected()
{
    return detcted_sig;
}


vector <double> QRSDetection::getFilter()
{
    return filter_sig;
}
